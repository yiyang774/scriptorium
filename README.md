...
| 层 | 位置 | 收什么 |
|---|---|---|
| 全局 | `projects/<host>/memory/*.md` + 同目录 `MEMORY.md` 索引 | 跨项目可复用的教训 / 偏好 / 方法论(≤25 条常驻,进会话自动加载索引) |
| 项目 | `<项目>/docs/superpowers/memory/*.md` + 该项目 `MEMORY.md` | 只有单个项目才用得上的状态与细节;进项目先读它自己的 `MEMORY.md` |
| 时间线 | `<项目>/docs/superpowers/journal/YYYY-MM-DD-<slug>.md` | 一任务一份;头部状态快照(每次覆写)+ 事件流(**只追加、绝不改写历史**——否则会"整理"掉不利记录,日志失去证据价值) |
...
**体检 / 工具**:
- `bin/mem-check [目录]` — 检查条数、索引一致性、有没有该下沉到项目内的错放在了全局
- `bin/mistakes [关键词]` — 从 MISTAKES.md 抽 active 项拼成简报片段
- **hook 兜底**:`SessionStart` / `PreCompact` / `Stop` 三个 hook 自动记录会话边界事件,所以"忘了记"这条借口不成立

⚠️ **本仓库只装机制,不装内容**:记忆条目、任务日志、错误档全部落在 `projects/` 目录,按 `.gitignore` 排除。**别 clone 我的记忆去用**——那是我自己踩坑攒的,直接搬会污染你的判断。你要建的是你自己的。
