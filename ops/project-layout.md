...
>
> **动因**：未按此规范前,同一项目常同时存在 `output/` 与 `outputs/`、实验数据散在六个不同目录、缓存与密钥躺在根目录。查一次结果要翻多处——本规范就是为了消除这个。
...
> **为什么要有脚本**：常见的结构性问题(悬空链接尤其是 `paperi` vs `paperI` 大小写、frontmatter name 与文件名不符、索引漏列、description 只写代号无内容)**肉眼扫不出**,得靠脚本。
...
> **判据的两个类型对照**——同一发现地的两条笔记,可能一条属跨项目教训(如 BLAS 超订、bash 3.2 兼容),一条只对该项目有意义(如某 commit 待办)——判据是"换项目还用得上"而不是"在哪发现的"。
>
> **跨层链接会断**:分层后原本的 `[[wikilink]]` 指向了别层的笔记。处置:跨层引用改成 `` `笔记名`(见 <路径>/) `` 的形式,同层内仍用 `[[ ]]`。
