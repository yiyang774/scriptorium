...
grep 和语义检索**结构上做不到**这件事，不是快慢之差。

实测（paperI_motivation 全仓，2026-08-05）：`impact cluster_bootstrap` 报 **68 个受影响符号**，
其中 **36 个所在文件从头到尾没出现过该符号名**——grep 永远找不到，**且这 36 个里 33 个是测试**。

这正对症 memory 里记的那次事故：聚合链最后一步藏雷、常数单测测不出
（`delta_c_bootstrap` 双重 bootstrap 把置信区间压窄 28 倍）。
...
paperI_motivation 全仓：**4,930 节点 / 12,423 边，索引耗时 1.3 秒，产物 16 MB**。
