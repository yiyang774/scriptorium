...
#### 实测效果（2026-08-05，同一脚本 A/B 对照，luna low）

被审对象：一个 10 行的本地日志脚本（读 stdin，若以 `rm` 开头则追加一行到 `~/.mylog`）。

| | A 组·不带 | B 组·带 |
|---|---|---|
| 报告问题数 | 6 | 3 |
| **威胁模型错配条数** | **4**（符号链接替换攻击、umask 致他人可读、命令含密码泄露、路径未加引号） | **0** |
| 真问题 | `rm*` 匹配过宽、"只是文本匹配非真审计" | `rm*` 匹配过宽、**换行会伪造多条日志**、写入失败无提示 |
| 结尾 | 给出一段预防性铺满的改写 | 显式写"不报命令注入等，本环境无实际攻击路径" |

**关键**：B 组不仅砍掉噪音，还**多逮到一个 A 组漏掉的真 bug**（stdin 含换行 → `echo`
写出两行 → 第二行伪装成独立日志记录）。注意力不被理论风险占用后，真问题反而看得更清。
...
- **L3 终检可按正交视角拆多路并发（建议，非硬要求）**：大体量 / 高风险产出（如整篇论文、跨文件重构）可拆 2–3 路互盲跑，各路简报只给**一个专攻面**（如"主张强度与诚实" vs "数字一致性与纪律"），别给同一份泛泛简报。**实测有效**（2026-08-05 Paper I 正文，两路各 30 条）：两路**独立逮到同一条致命项**（定理条件写错）互为佐证，其余基本不重叠——一路抓测试集污染与预注册泄漏，另一路抓算术矛盾与维度对不上。体量小就单路，别仪式化拆分。

### L1 · 执行（L0 主循环挑档：codex `gpt-5.6-luna` 主力 / Sonnet 深度 / Haiku 轻量）

> ⚠️ 简报里**必带**上方【环境与威胁模型】段——否则 codex 会写出预防性铺满的过度防御代码。
> **推理档：luna 一律 `max`**（2026-08-07 用户裁定；实测 `-c model_reasoning_effort="max"` 退出码 0 可跑通）。
> 执行与 L2 快审同档，不再分 low/medium/high——省档带来的时间收益不值得拿产出质量换。
...
> **形态(2026-08-05 用户裁定简化)**:**1 个 Codex `gpt-5.6-sol` + `ultra` + `-s read-only`,以 subagent 形式发起,有罪推定单审,spec 与 plan 同审,一次覆盖四维**。
>
...
> 实测（2026-08-05，guard 交付）：L3 曾把"字符串匹配可被有意规避"列为阻断项，
